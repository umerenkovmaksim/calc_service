...
func TestCalc(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.expression, func(t *testing.T) {
			...
			if err != nil {
				t.Errorf("Calc(%s) encountered error: %v", testCase.expression, err)
			} else if result != testCase.expected {
				t.Errorf("Calc(%s) = %v, expected %v", testCase.expression, result, testCase.expected)
			}
		})
	}
}
...
func TestCalcErrors(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase, func(t *testing.T) {
			...
			if err == nil {
				t.Errorf("Calc(%s) did not return an error as expected", testCase)
			}
		})
	}
}
